...
const author = async function(req, res) {
  ...
  const name = 'Jack Shumway';
  const pennKey = 'jshumway';
...
const song = async function(req, res) {
  ...
  connection.query(`
    SELECT * FROM
    Songs WHERE song_id = '${req.params.song_id}'
  `, (err, data) => {
    if (err || data.length === 0) {
      console.log(err);
      res.json({});
...
const album = async function(req, res) {
  ...
  connection.query(`
    SELECT * FROM
    Albums WHERE album_id = '${req.params.album_id}'
  `, (err, data) => {
    if (err || data.length === 0) {
      console.log(err);
      res.json({});
...
const albums = async function(req, res) {
  ...
  connection.query(`
    SELECT album_id, title, release_date, thumbnail_url FROM
    Albums order by release_date desc;
  `, (err, data) => {
    if (err || data.length === 0) {
      console.log(err);
      res.json({});
...
const album_songs = async function(req, res) {
  ...
  connection.query(`
    SELECT song_id, title, number, duration, plays FROM
    Songs where album_id = '${req.params.album_id}' order by number;
  `, (err, data) => {
    if (err || data.length === 0) {
      console.log(err);
      res.json({});
  ...
  if (!page) {
    ...
    connection.query(`
      SELECT 
        ...
        plays
      FROM
      Songs left join Albums on Songs.album_id = Albums.album_id
      order by plays desc;
    `, (err, data) => {
      if (err || data.length === 0) {
        console.log(err);
